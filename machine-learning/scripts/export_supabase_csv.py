...
O SQL espelha o vocabulário em machine-learning/dataset-referencia-glicnutri.md (paciente ativo,
...
def main() -> None:
    ...
    parser.add_argument(
        "--manifest",
        default=None,
        help="Opcional: gravar JSON com metadados reprodutíveis (linhas, colunas, hash SHA-256 do CSV).",
    )
    ...
    if args.manifest:
        ...
        manifest = {
            ...
            "since_ts": since_ts.isoformat() if hasattr(since_ts, "isoformat") else str(since_ts),
            "until_ts": until_ts.isoformat() if hasattr(until_ts, "isoformat") else str(until_ts),
            "output_path": out_abs,
            "row_count": int(len(df)),
            "columns": list(df.columns),
            "csv_sha256": sha256.hexdigest(),
        }
