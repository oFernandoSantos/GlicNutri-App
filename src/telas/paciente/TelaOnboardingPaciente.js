...
export default function PacienteOnboardingScreen({
    ...
    const timer = setTimeout(() => {
      navigation.reset({
        ...
        routes: [
          {
            name: 'HomePaciente',
            params: { usuarioLogado: completedPatient || usuarioLogado },
          },
        ],
      });
      onOnboardingFinished?.(completedPatient || usuarioLogado);
    }, 1200);

    return () => clearTimeout(timer);
  }, [completedPatient, finished, navigation, onOnboardingFinished, usuarioLogado]);
  ...
  async function finishOnboarding(skipped = false) {
    ...
    setSaving(true);
    const updatedPatient = await markPatientOnboardingSeen(usuarioLogado, {
      ...payload,
      skipped,
    });
    const nextPatient = updatedPatient || usuarioLogado;
    setCompletedPatient(nextPatient);
