...
import {
  ...
  isChatCompactLayout,
  loadMedicoChatReadAtForPatients,
  markMedicoChatRead,
...
function applyThreadPreviewToChat(
  ...
  const preview = buildMedicoThreadPreview(thread, { lastReadAt });
...
function buildChatItems(
  patients,
  summariesById,
  { preserveMessagesById, medicoReadAtByPatientId = {} } = {}
) {
  const items = (patients || []).map((patient) => {
    ...
    const preview =
      summary.preview ||
      buildMedicoThreadPreview(inboxThread, {
        lastReadAt: medicoReadAtByPatientId[patient.id],
      });
    ...
    return {
      ...
      lastReadAt: medicoReadAtByPatientId[patient.id] || null,
      patient,
    };
  });
...
export default function TelaMensagensMedico({ navigation, route }) {
  ...
  const medicoReadAtRef = useRef({});
  const [medicoReadAtByPatientId, setMedicoReadAtByPatientId] = useState({});
  const showTabBar = route?.name === 'MedicoMensagens';
  const chatPanelHeight = useMemo(() => {
    const readerHeader = insets.top + READER_BAR_HEIGHT;
    const tabBarSpace = showTabBar ? MEDICO_TAB_BAR_HEIGHT + MEDICO_TAB_BAR_SPACE + 16 : 16;
  ...
  useEffect(() => {
    carregarSessaoMedico()
      .then((session) => setMedicoSessaoPersistida(session))
      .catch(() => null);
  }, []);
  ...
  useEffect(() => {
    medicoReadAtRef.current = medicoReadAtByPatientId;
  }, [medicoReadAtByPatientId]);
  ...
  const mergeInboxPreviews = useCallback((patients, summaries, readAtOverride = null) => {
    ...
    const readAtMap = readAtOverride || medicoReadAtRef.current;

    return sortChatsByRecency(
      buildChatItems(patients, summariesById, {
        preserveMessagesById,
        medicoReadAtByPatientId: readAtMap,
      })
    );
  }, []);
  ...
  const buildChatActor = useCallback(() => {
    ...
    const resolvedMedicoId = medicoId || getMedicoId(base) || getMedicoId(usuarioLogado);
    const email =
      base?.email_medico ||
  ...
  const ensureChatSession = useCallback(async () => {
    const chatActor = buildChatActor();
    if (!chatActor.id_medico_uuid) {
      throw new Error('Medico sem identificador para abrir o chat.');
    }
    const token = await garantirSessaoRpcClinicaComPerfil(chatActor);
    if (!token) {
      throw new Error('Sessao clinica ausente. Saia do app e faca login novamente para usar o chat.');
    }
    return chatActor;
  }, [buildChatActor]);
  ...
  const loadInboxPreviewsForPatients = useCallback(
    async (patients, { fromIndex = 0, count = INBOX_PREVIEW_BATCH, chatActor = null } = {}) => {
      if (!medicoId || !patients?.length) return;

      const slice = patients.slice(fromIndex, fromIndex + count);
      const patientIds = slice.map((patient) => patient.id);
      if (!patientIds.length) return;

      const sessionActor = chatActor || buildChatActor();
      const [summaries, readMap] = await Promise.all([
        fetchCachedMedicoChatInbox(medicoId, patientIds, () =>
          fetchMedicoChatInboxForPatientIds(
            patientIds,
            medicoId,
        ...
        loadMedicoChatReadAtForPatients(medicoId, patientIds),
      ]);

      if (Object.keys(readMap).length) {
        setMedicoReadAtByPatientId((current) => ({ ...current, ...readMap }));
      }

      setChats((current) => {
        const mergedPatients = patientsRef.current || patients;
        const mergedReadAt = { ...medicoReadAtRef.current, ...readMap };
    ...
    [buildChatActor, mergeInboxPreviews, medicoId]
  );
    ...
    async ({ silent = false, bustCache = false } = {}) => {
      ...
      try {
        ...
        if (!medicoId) {
          ...
          setLoadError('Medico sem identificador para abrir o chat.');
          return;
        }
        ...
        try {
          ...
          sessionLoadError = sessionError?.message || 'Medico sem identificador para abrir o chat.';
        }
        ...
        try {
          patients = await listPatientsByDoctor(medicoId);
        } catch (patientError) {
        ...
        console.log('Erro ao carregar conversas do medico:', error);
        setLoadError(
  ...
  const loadActiveThread = useCallback(
    async (patientId, { silent = false } = {}) => {
      ...
      try {
        ...
        const chatActor = buildChatActor();
        const fetched = await fetchMedicoChatThreadForPatient(patientId, medicoId, {
          patientName,
          medicoName,
          limit: 200,
          rpcActor: chatActor,
        ...
        const viewingActive =
          ...
          chatsRef.current.find((chat) => chat.id === activeChatIdRef.current)?.patientId ===
            patientId;
        let readAt = medicoReadAtRef.current[patientId] || null;

        if (viewingActive && medicoId) {
          readAt = (await markMedicoChatRead(medicoId, patientId)) || readAt;
          if (readAt) {
            setMedicoReadAtByPatientId((current) => ({ ...current, [patientId]: readAt }));
          }
        }
    ...
    [buildChatActor, ensureChatSession, medicoId, medicoName]
  );
  ...
  const scheduleInboxRefresh = useCallback(
    ...
    [loadActiveThread, loadInbox, loadInboxPreviewsForPatients, medicoId]
  );
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      if (!activeChat?.patientId || !medicoId) return undefined;

      if (!sendingRef.current) {
        loadActiveThread(activeChat.patientId, { silent: true });
      }
      return undefined;
    }, [activeChatId, loadActiveThread, medicoId])
  );

  useEffect(() => {
    const activeChat = chatsRef.current.find((chat) => chat.id === activeChatId);
    const patientId = activeChat?.patientId;
    if (!patientId || !medicoId) return undefined;
  ...
  const handleSelectChat = useCallback(
    async (chatId) => {
      ...
      if (selected?.patientId && medicoId) {
        readAt = (await markMedicoChatRead(medicoId, selected.patientId)) || readAt;
        if (readAt) {
          setMedicoReadAtByPatientId((current) => ({
            ...current,
      ...
      .on(
        ...
        (payload) => {
          ...
          if (row?.texto && patientId && payload?.eventType === 'INSERT') {
            ...
            const entry = mapMedicoRealtimeChatRowToThreadEntry(
              row,
            ...
            if (entry) {
              if (sendingRef.current && entry.role === 'medico') {
                return;
              }

              const viewingThread = activeChat?.patientId === patientId;
              if (viewingThread && entry.role === 'user' && medicoId) {
                markMedicoChatRead(medicoId, patientId).then((readAt) => {
                  if (!readAt) return;
                  setMedicoReadAtByPatientId((current) => ({ ...current, [patientId]: readAt }));
                });
              }
              ...
              setChats((current) =>
                sortChatsByRecency(
                  current.map((chat) => {
                    if (chat.patientId !== patientId) return chat;
                    const nextThread = mergeMedicoChatMessageIntoThread(chat.messages || [], entry);
                    const lastReadAt =
  ...
  }, [loadActiveThread, medicoId, medicoName]);
  ...
  const appendAndPersistMessage = useCallback(
    async (text, chatId, threadOverride = null, { registroContext = null } = {}) => {
      ...
      const chatActor = buildChatActor();
      if (!chatActor.id_medico_uuid) {
        throw new Error('Medico sem identificador para enviar mensagem.');
      }
      ...
      const nextMessage = {
        ...normalizeMedicoThreadEntry(
          {
            id: `medico-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
            author: medicoName,
            role: 'medico',
            time: formatTimeNow(),
            text: trimmed,
      ...
      const normalizedThread = attachRegistroContextToThreadMessages(
        savedThread.map((item) =>
          normalizeMedicoThreadEntry(item, {
            medicoName,
            patientName: chatSnapshot.patientName,
          })
        ),
    ...
    [buildChatActor, ensureChatSession, medicoName, medicoId]
  );
  ...
  async function handleSend() {
    ...
    if (!chatActor.id_medico_uuid) {
      Alert.alert(
        ...
        'Sua conta de medico nao foi identificada. Saia e entre novamente.'
      );
      return;
    }
    ...
    try {
      ...
      console.log('Erro ao enviar resposta do medico:', error);
              ...
              {visibleChats.map((chat) => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    activeOpacity={0.9}
                  >
                    <AvatarBadge name={chat.patientName} size={42} subtle theme={patientTheme} />
            ...
            >
              ...
              <View style={styles.threadHeader}>
                <View style={styles.threadIdentity}>
                  <AvatarBadge name={activeChat.patientName} size={52} theme={patientTheme} />
              ...
              >
                {threadLoading && !(activeChat.messages || []).length ? (
                  ...
                  (activeChat.messages || []).map((message) => {
                    const mine = isMedicoProfessionalRole(message.role);
