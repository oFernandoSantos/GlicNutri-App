...
import {
  loadDocumentosClinicosMedico,
  saveDocumentosClinicosMedico,
} from '../../servicos/servicoDocumentosClinicosMedico';
...
function normalizePatientForProntuario(patient) {
  ...
  return {
    ...
    notes: patient.notes || raw.observacoes || 'Paciente vinculado ao acompanhamento clínico.',
...
export default function TelaProntuarioPacienteMedico({ navigation, route }) {
  ...
  const [savingDocumentos, setSavingDocumentos] = useState(false);
  const [documentosMsg, setDocumentosMsg] = useState(null);
  const [receitas, setReceitas] = useState([]);
  const [atestados, setAtestados] = useState([]);
  const [receitaTitulo, setReceitaTitulo] = useState('Receita médica');
  const [receitaMedicamentos, setReceitaMedicamentos] = useState('');
  const [receitaPosologia, setReceitaPosologia] = useState('');
  const [receitaObs, setReceitaObs] = useState('');
  const [atestadoTitulo, setAtestadoTitulo] = useState('Atestado médico');
  ...
  const [medicoSessaoPersistida, setMedicoSessaoPersistida] = useState(null);

  useEffect(() => {
    let active = true;
    carregarSessaoMedico()
      .then((sessao) => {
        if (active) setMedicoSessaoPersistida(sessao || null);
      })
      .catch(() => {
        if (active) setMedicoSessaoPersistida(null);
      });
    return () => {
      active = false;
    };
  ...
  }, [medicoRpcActor?.id_medico_uuid, paciente?.id, pacienteId, patientRecord?.id_paciente_uuid]);
  ...
  const loadGlucose12hChart = useCallback(
    async (patientId, { force = false } = {}) => {
      ...
      try {
        if (!medicoRpcActor?.id_medico_uuid) return;
        await garantirSessaoRpcClinicaComPerfil(medicoRpcActor).catch(() => {});
  ...
  useEffect(() => {
    if (!chartPatientId || !medicoRpcActor?.id_medico_uuid) return;
    if (glucose12hLoadPatientRef.current === chartPatientId) return;
    loadGlucose12hChart(chartPatientId);
  }, [chartPatientId, loadGlucose12hChart, medicoRpcActor?.id_medico_uuid]);
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (medicoRpcActor?.id_medico_uuid) {
          await garantirSessaoRpcClinicaComPerfil(medicoRpcActor).catch(() => {});
        }
  ...
  }, [chartPatientId, medicoRpcActor?.id_medico_uuid, paciente?.id, patientRecord?.id_paciente_uuid]);
  ...
  const loadDocumentos = useCallback(async () => {
    ...
    try {
      ...
      setDocumentosMsg(null);
      const bundle = await loadDocumentosClinicosMedico(effectivePacienteId, medicoId);
      setReceitas(bundle.receitas || []);
      setAtestados(bundle.atestados || []);
    } catch (error) {
      setDocumentosMsg({
        tipo: 'erro',
        texto: error?.message || 'Não foi possível carregar receitas e atestados.',
      });
    } finally {
      setLoadingDocumentos(false);
    }
  }, [effectivePacienteId, medicoId]);

  useEffect(() => {
    if (activeTab === 'documentos' && effectivePacienteId && medicoId) {
      loadDocumentos();
    }
  }, [activeTab, effectivePacienteId, medicoId, loadDocumentos]);
  ...
  const reloadPatientExperience = useCallback(async () => {
    ...
    try {
      setExperienceLoadError('');
      if (medicoRpcActor?.id_medico_uuid) {
        await garantirSessaoRpcClinicaComPerfil(medicoRpcActor);
      }
  ...
  const loadHistorico = useCallback(async () => {
    ...
    try {
      ...
      const hist = await fetchConsultasHistorico(effectivePacienteId, medicoId, 30);
  ...
  useEffect(() => {
    if (
      ...
      !effectivePacienteId ||
      !patientExperience ||
      !medicoRpcActor?.id_medico_uuid
    ) {
      return;
    }
    loadRegistrosRef.current?.();
  }, [activeTab, effectivePacienteId, medicoRpcActor?.id_medico_uuid, patientExperience]);

  function clearReceitaDraft() {
    setReceitaTitulo('Receita médica');
    setReceitaMedicamentos('');
    setReceitaPosologia('');
    setReceitaObs('');
  }

  function clearAtestadoDraft() {
    setAtestadoTitulo('Atestado médico');
    setAtestadoDias('');
    setAtestadoCid('');
    setAtestadoConteudo('');
  }
  ...
  function addReceita() {
    if (!receitaMedicamentos.trim() && !receitaPosologia.trim()) {
      setDocumentosMsg({ tipo: 'erro', texto: 'Informe medicamentos ou posologia da receita.' });
      return;
    }
    const item = {
      id: `receita-${Date.now()}`,
      titulo: receitaTitulo.trim() || 'Receita médica',
      medicamentos: receitaMedicamentos.trim(),
      posologia: receitaPosologia.trim(),
      observacoes: receitaObs.trim(),
      createdAt: new Date().toISOString(),
    };
    setReceitas((curr) => [item, ...curr]);
    clearReceitaDraft();
    setDocumentosMsg(null);
  }

  function removeReceita(id) {
    setReceitas((curr) => curr.filter((item) => item.id !== id));
  }

  function addAtestado() {
    if (!atestadoConteudo.trim()) {
      setDocumentosMsg({ tipo: 'erro', texto: 'Descreva o conteúdo do atestado.' });
      return;
    }
    const item = {
      id: `atestado-${Date.now()}`,
      titulo: atestadoTitulo.trim() || 'Atestado médico',
      diasAfastamento: atestadoDias.trim(),
      cid: atestadoCid.trim(),
      conteudo: atestadoConteudo.trim(),
      createdAt: new Date().toISOString(),
    };
    setAtestados((curr) => [item, ...curr]);
    clearAtestadoDraft();
    setDocumentosMsg(null);
  }

  function removeAtestado(id) {
    setAtestados((curr) => curr.filter((item) => item.id !== id));
  }

  async function persistDocumentos(nextReceitas = receitas, nextAtestados = atestados) {
    if (!effectivePacienteId || !medicoId) return;
    try {
      setSavingDocumentos(true);
      setDocumentosMsg(null);
      await saveDocumentosClinicosMedico(effectivePacienteId, medicoId, {
        receitas: nextReceitas,
        atestados: nextAtestados,
      });
      setDocumentosMsg({ tipo: 'sucesso', texto: 'Documentos salvos.' });
    } catch (error) {
      setDocumentosMsg({
        tipo: 'erro',
        texto: error?.message || 'Não foi possível salvar os documentos.',
      });
    } finally {
      setSavingDocumentos(false);
    }
  }

  async function saveReceitas() {
    await persistDocumentos(receitas, atestados);
  }

  async function saveAtestados() {
    await persistDocumentos(receitas, atestados);
  }
  ...
  return (
    <View style={styles.container}>
        ...
        <View style={styles.headerCard}>
          <View style={styles.headerMain}>
            <View style={styles.identityRow}>
              <AvatarBadge name={currentPatient.name} size={56} theme={patientTheme} />
          ...
          <FilterTabs items={detailTabs} active={activeTab} onChange={setActiveTab} compact scrollable fill theme={patientTheme} />
        </View>
      </View>
      ...
      >
        ...
        {activeTab === 'evolucao' ? (
          <View style={styles.pageGap}>
            ...
            <SectionCard>
              ...
              <LabeledInput label="A — Avaliação clínica / Diagnóstico nutricional" value={evolucaoAvaliacao} onChange={setEvolucaoAvaliacao} placeholder="Avaliação do medico..." multiline />
        ...
        {/* ── TAB: RECEITAS E ATESTADOS ─────────────────────────────────── */}
        {activeTab === 'documentos' ? (
          <View style={styles.pageGap}>
            <SectionCard>
              <Text style={styles.sectionTitle}>Receitas do paciente</Text>
              <Text style={styles.sectionHelper}>
                Prescrições e orientações farmacológicas emitidas no acompanhamento.
              </Text>
              {loadingDocumentos ? (
                <View style={styles.inlineStatus}>
                  <ActivityIndicator color={patientTheme.colors.primaryDark} />
                  <Text style={styles.inlineStatusText}>Carregando receitas...</Text>
                </View>
              ) : null}
              {documentosMsg ? <InlineMsg msg={documentosMsg} /> : null}
              <View style={styles.mealList}>
                {receitas.length ? (
                  receitas.map((item) => (
                    <View key={item.id} style={styles.mealCard}>
                      <View style={styles.mealTop}>
                        <Text style={styles.mealTime}>{formatDate(item.createdAt)}</Text>
                        <Text style={styles.mealTitle}>{item.titulo}</Text>
                        <TouchableOpacity onPress={() => removeReceita(item.id)} style={{ marginLeft: 'auto' }}>
                          <Ionicons name="trash-outline" size={16} color={patientTheme.colors.danger} />
                        </TouchableOpacity>
                      </View>
                      {item.medicamentos ? (
                        <Text style={styles.mealSummary}>Medicamentos: {item.medicamentos}</Text>
                      ) : null}
                      {item.posologia ? (
                        <Text style={styles.mealSummary}>Posologia: {item.posologia}</Text>
                      ) : null}
                      {item.observacoes ? (
                        <Text style={styles.mealSummary}>Obs.: {item.observacoes}</Text>
                      ) : null}
                    </View>
                  ))
                ) : (
              ...
              <PrimaryButton
                label="Salvar receitas"
                icon="save-outline"
                onPress={saveReceitas}
                loading={savingDocumentos}
                variant="brand"
              />
            </SectionCard>

            <SectionCard>
              <Text style={styles.sectionTitle}>Nova receita</Text>
              <LabeledInput
                label="Título"
                value={receitaTitulo}
                onChange={setReceitaTitulo}
                placeholder="Ex: Receita médica"
              />
              <LabeledInput
                label="Medicamentos"
                value={receitaMedicamentos}
                onChange={setReceitaMedicamentos}
                placeholder="Ex: Metformina 850 mg, Losartana 50 mg"
                multiline
              />
              <LabeledInput
                label="Posologia"
                value={receitaPosologia}
                onChange={setReceitaPosologia}
                placeholder="Ex: 1 comprimido após café e jantar"
                multiline
              />
              <LabeledInput
                label="Observações"
                value={receitaObs}
                onChange={setReceitaObs}
            ...
            <SectionCard>
              <Text style={styles.sectionTitle}>Atestados médicos</Text>
              <Text style={styles.sectionHelper}>
                Registre atestados e declarações clínicas do paciente.
              </Text>
              <View style={styles.mealList}>
                {atestados.length ? (
                  atestados.map((item) => (
                    <View key={item.id} style={styles.mealCard}>
                      <View style={styles.mealTop}>
                        <Text style={styles.mealTime}>{formatDate(item.createdAt)}</Text>
                        <Text style={styles.mealTitle}>{item.titulo}</Text>
                        <TouchableOpacity onPress={() => removeAtestado(item.id)} style={{ marginLeft: 'auto' }}>
                          <Ionicons name="trash-outline" size={16} color={patientTheme.colors.danger} />
                        </TouchableOpacity>
                      </View>
                      {item.diasAfastamento ? (
                        <Text style={styles.mealSummary}>Afastamento: {item.diasAfastamento} dia(s)</Text>
                      ) : null}
                      {item.cid ? <Text style={styles.mealSummary}>CID: {item.cid}</Text> : null}
                      <Text style={styles.mealSummary}>{item.conteudo}</Text>
                    </View>
                  ))
                ) : (
                  <Text style={styles.emptyText}>Nenhum atestado registrado ainda.</Text>
                )}
              </View>
              <PrimaryButton
                label="Salvar atestados"
                icon="save-outline"
                onPress={saveAtestados}
                loading={savingDocumentos}
                variant="brand"
              />
            </SectionCard>
            ...
            <SectionCard>
              <Text style={styles.sectionTitle}>Novo atestado</Text>
              <LabeledInput
                label="Título"
                value={atestadoTitulo}
                onChange={setAtestadoTitulo}
              ...
              <LabeledInput
                ...
                value={atestadoConteudo}
                onChange={setAtestadoConteudo}
                placeholder="Texto do atestado para o paciente"
                multiline
              />
              <PrimaryButton
