...
export default function TelaAgendaMedico({ navigation, route }) {
  ...
  const agendaLoadGuardRef = useRef(criarGuardiaoCarregamentoInicial());
  ...
  const [medicoAvailability, setMedicoAvailability] = useState([]);
  ...
  const loadAgenda = useCallback(async () => {
    ...
    try {
      ...
      const [items, pendingRequests, linkedPatients, availability] = await Promise.all([
        ...
        listMedicoAvailability(medicoId),
      ]);
      setConsultas(items || []);
      setRequests(pendingRequests || []);
      setPatients(linkedPatients || []);
      setMedicoAvailability(availability || []);
    } catch (error) {
  ...
  useEffect(() => {
    (async () => {
      await loadAgenda();
      agendaLoadGuardRef.current.marcarCarregado();
    })();
  }, [loadAgenda]);

  useEffect(() => {
    const unsubscribe = navigation.addListener('focus', () => {
      if (agendaLoadGuardRef.current.deveIgnorarCarregamentoFocus()) return;
      loadAgenda();
    });
    return unsubscribe;
  ...
  useEffect(() => {
    if (activeAgendaTab !== 'solicitacoes' || !medicoId) return;
  ...
  const summary = useMemo(() => {
    ...
    return [
      { id: 'today', label: 'Consultas Hoje', value: placeholder ?? String(consultasHoje.length), icon: 'today-outline', accent: MEDICO_KPI_BLUE },
      { id: 'week', label: 'Esta Semana', value: placeholder ?? String(consultasSemana.length), icon: 'calendar-outline', accent: MEDICO_KPI_BLUE },
      { id: 'month', label: 'Este Mês', value: placeholder ?? String(consultasMes.length), icon: 'calendar-number-outline', accent: MEDICO_KPI_BLUE },
  ...
  const medicoAgendaSlots = useMemo(() => {
    const generated = generateSlotsForNextDays(medicoAvailability, { days: 30 });
    return markSlotsWithBooking(generated, consultas);
  }, [consultas, medicoAvailability]);

  const availableDays = useMemo(
    () =>
      groupSlotsByDay(medicoAgendaSlots)
        .map((day) => ({
          ...day,
  ...
  const slotsForSelectedDate = useMemo(() => {
    ...
    const day = groupSlotsByDay(medicoAgendaSlots).find((item) => item.dateKey === formDateKey);
    return day?.slots || [];
  }, [formDateKey, medicoAgendaSlots]);
      ...
      <View key={consulta.id} style={styles.consultaRow}>
        ...
        >
          ...
          <View style={styles.consultaBody}>
            <AvatarBadge name={patientName} size={38} subtle theme={patientTheme} />
                ...
                return (
                  <View key={request.id} style={[styles.flatCard, styles.requestCard]}>
                    <View style={styles.requestLeft}>
                      <AvatarBadge name={pacienteNome} size={40} subtle theme={patientTheme} />
        ...
        <View style={styles.confirmOverlay} pointerEvents="box-none">
          ...
          <View style={styles.consultaDetailCard}>
            ...
            <View style={styles.confirmActions}>
              ...
              <TouchableOpacity
                style={[
                  styles.confirmPrimaryButtonMedico,
