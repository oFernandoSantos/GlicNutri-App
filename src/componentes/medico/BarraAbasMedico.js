...
const abasPrincipais = [
  ...
  { rota: 'MedicoPacientes', rotulo: 'Pacientes', icone: 'people-outline' },
  { rota: 'HomeMedico', rotulo: 'Início', icone: 'home-outline' },
  { rota: 'MedicoMensagens', rotulo: 'Mensagens', icone: 'chatbubbles-outline' },
  { rota: 'MedicoRelatorios', rotulo: 'Relatórios', icone: 'bar-chart-outline' },
];

const rodapeWebFixo = Platform.OS === 'web' ? { position: 'fixed', zIndex: 850 } : null;

export default function BarraAbasMedico({ navigation, rotaAtual, usuarioLogado }) {
  ...
  function navegar(rota) {
    ...
    navigateMedicoTab(navigation, rota, usuarioLogado);
  }
...
const styles = StyleSheet.create({
  ...
  abaInner: { alignItems: 'center', justifyContent: 'center' },
  iconeAbaWrap: { alignItems: 'center', borderRadius: 20, height: 34, justifyContent: 'center', width: 44 },
  iconeAbaInner: { alignItems: 'center', justifyContent: 'center' },
  iconeAbaWrapAtivo: {
  ...
  iconeAbaInnerAtivo: { alignItems: 'center', gap: 4, justifyContent: 'center', transform: [{ translateY: -2 }] },
  rotuloAba: { marginTop: 1, fontSize: 10, fontWeight: '500', color: '#8B95A1' },
